...
class GuidedAAC(BaseAAC):
    ...
    This implementation is loosely referred to as 'guided policy search' after algorithm described in paper
    by S. Levine and P. Abbeel `Learning Neural Network Policies with Guided PolicySearch under Unknown Dynamics`
    in a sense that exploits idea of fitting a 'local' (here - single episode) oracle for the environment with
    generally unknown dynamics and use actions demonstrated by it to optimize the trajectory distribution for the training agent.

    Note that this particular implementation of expert does not provide
    a complete action-state space trajectory for agent to follow.
